...
> **当前版本：v1.8.4**（版本号见 `app/core/config.py` 的 `APP_VERSION`，界面顶栏同步显示；每次功能/修复发布递增）

> **核心修复（v1.8.4）**：① 静态资源版本化（`?v=版本号` 缓存击穿）——根治 OTA 更新后界面仍是旧布局的问题；② **幻觉字幕过滤**——纯音乐/无人声视频不再显示"字幕由 Amara.org 提供"等 whisper 幻觉水印，直接显示"未检测到人声"；③ 字幕缓存随转写时间自动失效（去词/重测后不再残留旧字幕）。
...
> **在线更新（v1.8.0）**：设置页新增「在线更新」面板。更新包只覆盖 `app/` 目录，不碰 `data/`、`logs/`、模型、视频和 exe；下载后校验 SHA256，退出软件后自动覆盖并重启。`latest.json` 是版本清单，app 小包随 `updates/` 目录提交到 main 分支，通过 GitHub raw 直链云更新；完整绿色包随本地发布包分发，可另存到 GitHub Releases（网页上传大文件有 25MB 限制，app 小包不受影响）。
...
- **本地转写**：faster-whisper（Whisper large-v3）+ 统一 16kHz 音轨预处理 + VAD 放宽参数 + 空结果自动重试；NVIDIA 显卡 GPU 加速；AMD/Intel/无独显自动降级 CPU int8，全程离线、数据不出本机
- **双层违禁词检测**：
  - 归一化精确层：繁简/全角/标点归一，可命中"第 一 名"这类分词规避写法
  - 拼音谐音层："蕞好"→"最好" 等谐音替换也能命中
- **精确定位（词级时间戳）**：命中时间取 whisper 词级时间戳（±0.2s，旧数据自动回退内插），每条命中给出 视频文件 / 时间点 / 命中词 / 完整口播句，点击即在播放器中跳到该秒回放
- **一键去除违禁词**：在命中列表勾选要去除的词 → ffmpeg 精确剪掉对应音画片段（仅 mp4）→ **产物时长校验**（剪辑未生效立即报错，绝不覆盖原文件）→ 自动备份原文件到同目录 → 成品文件名保持不变、参数保持一致
- **词库管理**：内置 4 分类 88 词种子库（广告法极限词/医疗功效/金融风险/平台违禁），支持增删改、批量导入、启停用；改词库后一键重新检索（无需重新转写）
- **报告导出**：Excel 明细报告、SRT 字幕文件
- **在线更新**：`latest.json` + `updates/` 仓库直链；检查/下载 app 小包 + SHA256 校验 + 退出后自动覆盖 `app/` 并重启；设置页可开关自动检查和国内镜像
...
├── updates/                  # app 在线更新小包（随 main 分支发布）
├── tests/                    # 测试与工具脚本
├── scripts/                  # 构建/打包/发布脚本
...
- **某视频"未检测到人声"**：该视频是纯音乐/无人声；whisper 幻觉水印（"字幕由 Amara.org 提供"等）已自动过滤
- **浏览器无法播放某格式**（如 mkv/avi）：时间点仍准确，可复制时间到本地播放器定位；建议用 mp4
- **去词后视频变短了**：勾选的片段（含前后 0.3 秒缓冲）已被剪掉；原视频已移入回收站（可恢复）
